...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/europacash-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    MONEROD_EXEC=europacashd.exe
fi
...
$QMAKE ../europacash-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 
